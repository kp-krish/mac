...
def search(request):
    ...
    if len(query)<4:
        params = {'msg': 'please make sure to enter relevent serach query'}
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        phone = request.POST.get('phone', '')
        order = Order(item_json=items_json, name=name, email=email, address=address, city=city,
        ...
        sid = order.order_id
        print(sid)
        return render(request, 'shop/checkout.html', {'thank': thank, 'sid': sid})
    print(sid)
